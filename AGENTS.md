...
- `src/daemon.rs`: POC Unix socket server, Linux SO_PEERCRED support, macOS LOCAL_PEERCRED/getpeereid UID checks, request-level UID mismatch rejection, required-UID enforcement for `get_secret`, and permission tests
- `src/run_cmd.rs`: POC process spawning with daemon request, required UID field, and env injection test
